...
#define STARTING_IBEACONLEVEL 99.0f
#define STARTING_CO2LEVEL 0x00
#define STARTING_SOUNDLEVEL 0x00
#define STARTING_HVACSTATUS NORM
...
SemaphoreHandle_t xMaxco2Semaphore;

uint co2data = STARTING_CO2LEVEL;
uint co2data_tsk = STARTING_CO2LEVEL;

//uint reportedco2 = STARTING_CO2LEVEL;
...
void uart_rx_task(void *arg)
{
    ...
    while (1)
    {
        ...
        if (rxBytes > 0)
        {
            ...
            xSemaphoreTake(xMaxco2Semaphore, portMAX_DELAY);
            co2data_tsk = co2;
            xSemaphoreGive(xMaxco2Semaphore);
        }
    ...
    while (NETWORK_ATTEMPTING_RECONNECT == rc || NETWORK_RECONNECTED == rc || SUCCESS == rc)
    {
        ...
        if (SUCCESS == rc)
        {
            ...
            rc = aws_iot_shadow_add_reported(JsonDocumentBuffer, sizeOfJsonDocumentBuffer, 3,
                                             &soundHandler, &co2Handler, &hvacStatusActuator);
            if (SUCCESS == rc)
            {
...
void app_main()
{
    ...
    xMaxco2Semaphore = xSemaphoreCreateMutex();
